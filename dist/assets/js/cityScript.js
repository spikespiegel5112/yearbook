...
citySelector.pc[0] = new Array("北京", "东城|西城|崇文|宣武|朝阳|丰台|石景山|海淀|门头沟|房山|通州|顺义|昌平|大兴|平谷|怀柔|密云|延庆");
citySelector.pc[1] = new Array("上海", "黄浦|卢湾|徐汇|长宁|静安|普陀|闸北|虹口|杨浦|闵行|宝山|嘉定|浦东|金山|松江|青浦|南汇|奉贤|崇明");
...
citySelector.hotCity = ["北京", "上海", "深圳", "无锡"];
...
citySelector.cityInit = function (input) {
    ...
    $("#" + input).click(function () {
        ...
        $("#js_cityBox").on("click", ".js_cityName", function (e) {
            e.stopPropagation();
            var $this=$(this);
            if ($this.closest('ul').siblings('span').length==0) {
                var pcHtml=$this.html();
                $("#" + _input).siblings('input').val(pcHtml);
                $("#js_cityBox").remove();
            }else{
                var pcHtml=$this.parent().siblings().text()+' - '+$this.html();
                $("#" + _input).siblings('input').val(pcHtml);
                $("#js_cityBox").remove();
            }
        });
    ...
    setTimeout(function() {
        $(document).click(function (e) {
            if ($(e.target).closest('.city-box').length == 0 && $(e.target).closest('#' + input).length == 0) {
                $('.city-box').detach();
            }
        });
    }, 500);
